...
const TaskSessionsView: React.FC = () => {
  ...
  const filteredSessions = sessions.filter(session => {
    ...
    return (
      session.task_title?.toLowerCase().includes(query) ||
      session.task_id.toString().includes(query) ||
      session.status.toLowerCase().includes(query) ||
      session.working_dir.toLowerCase().includes(query)
    );
  });
    ...
    <Box>
      ...
      >
        <Stack direction="row" spacing={2} alignItems="center">
          <TextField
            fullWidth
            placeholder="Search sessions by task, ID, status, or path..."
            value={searchQuery}
...
export default TaskSessionsView;
