...
const Hooks: React.FC = () => {
  ...
  const handleEnableAll = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/projects/${selectedProject.id}/hooks/enable-all`
      );
      await fetchHooks();
      if (response.data.errors && response.data.errors.length > 0) {
        setError(`Enabled ${response.data.enabled_count} hooks with some errors: ${response.data.errors.join(', ')}`);
      }
    } catch (err: any) {
      setError('Failed to enable all hooks: ' + (err.response?.data?.detail || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDisableAll = async () => {
    if (!selectedProject?.id) return;
    if (!window.confirm('Are you sure you want to disable all enabled hooks?')) return;

    setLoading(true);
    setError(null);
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/projects/${selectedProject.id}/hooks/disable-all`
      );
      await fetchHooks();
      if (response.data.errors && response.data.errors.length > 0) {
        setError(`Disabled ${response.data.disabled_count} hooks with some errors: ${response.data.errors.join(', ')}`);
      }
    } catch (err: any) {
      setError('Failed to disable all hooks: ' + (err.response?.data?.detail || err.message));
    } finally {
      setLoading(false);
    }
  };
