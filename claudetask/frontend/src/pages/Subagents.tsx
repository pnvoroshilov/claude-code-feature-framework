...
const Subagents: React.FC = () => {
  ...
  const [newSubagentCategory, setNewSubagentCategory] = useState('Development');
  const [newSubagentType, setNewSubagentType] = useState('');
  const [newSubagentConfig, setNewSubagentConfig] = useState('');
  const [newSubagentTools, setNewSubagentTools] = useState('');
  ...
  const handleCreateSubagent = async () => {
    ...
    setError(null);
    try {
      const toolsArray = newSubagentTools.split(',').map(t => t.trim()).filter(t => t.length > 0);
      ...
      await axios.post(
        `${API_BASE_URL}/api/projects/${activeProjectId}/subagents/create`,
        {
          ...
          category: newSubagentCategory,
          subagent_type: newSubagentType,
          config: newSubagentConfig,
          tools_available: toolsArray.length > 0 ? toolsArray : null,
        }
      );
      setCreateDialogOpen(false);
      setNewSubagentName('');
      setNewSubagentDescription('');
      setNewSubagentCategory('Development');
      setNewSubagentType('');
      setNewSubagentConfig('');
      setNewSubagentTools('');
      await fetchSubagents(); // Refresh subagents list
    } catch (err: any) {
      setError('Failed to create subagent: ' + (err.response?.data?.detail || err.message));
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <Box p={3}>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Create Custom Subagent</DialogTitle>
        <DialogContent>
          <Box display="flex" flexDirection="column" gap={2} mt={1}>
            <TextField
              ...
              fullWidth
              required
              helperText="A unique name for your custom subagent"
            />
            <TextField
              label="Description"
              ...
              multiline
              rows={2}
              helperText="Describe what this subagent does"
            />
            <FormControl fullWidth>
              <InputLabel>Category</InputLabel>
              <Select
                value={newSubagentCategory}
                onChange={(e) => setNewSubagentCategory(e.target.value)}
                label="Category"
              >
                {categories.map((cat) => (
            ...
            <TextField
              ...
              fullWidth
              required
              helperText="The subagent_type identifier (e.g., 'my-custom-agent')"
            />
            <TextField
              label="Configuration"
              value={newSubagentConfig}
              onChange={(e) => setNewSubagentConfig(e.target.value)}
              fullWidth
              required
              multiline
              rows={6}
              helperText="Agent configuration and instructions in markdown format"
            />
            <TextField
              label="Available Tools (comma-separated)"
              value={newSubagentTools}
              onChange={(e) => setNewSubagentTools(e.target.value)}
              fullWidth
              helperText="e.g., Read, Write, Edit, Bash, Grep"
            />
          </Box>
        </DialogContent>
        <DialogActions>
