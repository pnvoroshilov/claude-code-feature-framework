...
const Skills: React.FC = () => {
  ...
  const handleEnableAll = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/projects/${selectedProject.id}/skills/enable-all`
      );
      await fetchSkills(); // Refresh skills list
      if (response.data.errors && response.data.errors.length > 0) {
        setError(`Enabled ${response.data.enabled_count} skills with some errors: ${response.data.errors.join(', ')}`);
      }
    } catch (err: any) {
      setError('Failed to enable all skills: ' + formatErrorMessage(err));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDisableAll = async () => {
    if (!selectedProject?.id) return;
    if (!window.confirm('Are you sure you want to disable all enabled skills?')) return;

    setLoading(true);
    setError(null);
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/projects/${selectedProject.id}/skills/disable-all`
      );
      await fetchSkills(); // Refresh skills list
      if (response.data.errors && response.data.errors.length > 0) {
        setError(`Disabled ${response.data.disabled_count} skills with some errors: ${response.data.errors.join(', ')}`);
      }
    } catch (err: any) {
      setError('Failed to disable all skills: ' + formatErrorMessage(err));
    } finally {
      setLoading(false);
    }
  };
