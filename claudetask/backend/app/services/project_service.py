...
from ..models import Project, Agent, Task, DefaultSkill, ProjectSkill
...
class ProjectService:
    ...
    async def initialize_project(
        ...
        # Settings are now part of Project model (merged)
        # Default settings values are already set in the Project model columns
...
"""
    ...
    async def regenerate_claude_md(db: AsyncSession, project_id: str):
        ...
        # Get project from database (settings merged into Project model)
        result = await db.execute(select(Project).where(Project.id == project_id))
